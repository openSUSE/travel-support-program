...
feature "Budget limits", "" do
  ...
  scenario "Choosing the currency" do
    ...
    visit request_path(requests(:wedge_for_party))
    click_link "Edit"
    page.should have_select "request_expenses_attributes_0_approved_currency", :with_options => %w(EUR)
    page.should_not have_select "request_expenses_attributes_0_approved_currency", :with_options => %w(USD)
  end
end
