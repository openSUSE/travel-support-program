...
describe 'Requester' do
  ...
  context 'when managing event emails' do
    context 'when an event organizer' do
      it { is_expected.not_to be_able_to(:create, EventEmail.new(event_id: events(:dagobah_camp).id)) }
      it { is_expected.to be_able_to(:create, EventEmail.new(event_id: events(:party).id)) }
      it { is_expected.to be_able_to(:read, event_emails(:party_info)) }
    end

    context 'when not an event organizer' do
  ...
  context 'when managing their own requests' do
    ...
    it { is_expected.to be_able_to(:read, requests(:luke_for_party)) }
    it { is_expected.to be_able_to(:read, requests(:luke_for_yavin)) }
    it { is_expected.to be_able_to(:update, requests(:luke_for_party)) }
    it { is_expected.not_to be_able_to(:update, requests(:luke_for_yavin)) }
    it { is_expected.to be_able_to(:destroy, requests(:luke_for_party)) }
  ...
  context "when looking into other's requests" do
    context 'when an event organizer' do
      it { is_expected.not_to be_able_to(:read, requests(:wedge_for_yavin)) }
  ...
  context 'when asking for a correct reimbursement' do
    it { is_expected.to be_able_to(:create, requests(:luke_for_yavin).build_reimbursement) }
  end

  context 'when asking for an incorrect reimbursement' do
    let(:user) { users(:wedge) }

    it { is_expected.not_to be_able_to(:create, requests(:luke_for_yavin).build_reimbursement) }
    it { is_expected.not_to be_able_to(:create, requests(:wedge_for_yavin).build_reimbursement) }
  end

  context 'when managing a reimbursement' do
    ...
    context "when trying to look into other's reimbursements" do
      let(:user) { users(:wedge) }

      it { is_expected.not_to be_able_to(:read, reimbursement) }
      it { is_expected.not_to be_able_to(:update, reimbursement) }
      it { is_expected.not_to be_able_to(:submit, reimbursement) }
  ...
  context 'when adding comments to their own requests' do
  ...
  context 'when accessing public comments on their own requests' do
  ...
  context 'when accessing private comments on their own requests' do
  ...
  context "when adding comments to other's requests" do
    it { is_expected.not_to be_able_to(:create, requests(:wedge_for_party).comments.build) }
  end

  context "when accessing comments on other's requests" do
    let(:comment) { requests(:wedge_for_yavin).comments.create(body: 'whatever') }
  ...
  context 'when they manage attachments to their own reimbursements' do
  ...
  context 'when managing their own shipments' do
    ...
    it { is_expected.to be_able_to(:update, requests(:luke_costumes_for_party)) }
    it { is_expected.to be_able_to(:destroy, requests(:luke_costumes_for_party)) }
    it { is_expected.to be_able_to(:cancel, requests(:luke_costumes_for_party)) }
    it { is_expected.not_to be_able_to(:adjust_state, requests(:luke_costumes_for_party)) }
  end
  ...
  context 'when managing their own requested shipments' do
    let(:user) { users(:wedge) }
    ...
    it { is_expected.to be_able_to(:cancel, requests(:wedge_costumes_for_party)) }
    it { is_expected.not_to be_able_to(:adjust_state, requests(:wedge_costumes_for_party)) }
  end
end
